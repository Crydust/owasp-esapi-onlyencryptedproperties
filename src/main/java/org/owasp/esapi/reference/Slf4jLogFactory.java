...
/*
 * Modified from JavaLogFactory for Slf4j --Kristof Neirynck 25/9/2013
 */
 ...
 * @see org.owasp.esapi.reference.Slf4jLogFactory.JavaLogger
 */
public class Slf4jLogFactory implements LogFactory {
    ...
    public static LogFactory getInstance() {
        if ( singletonInstance == null ) {
            synchronized ( Slf4jLogFactory.class ) {
                if ( singletonInstance == null ) {
                    singletonInstance = new Slf4jLogFactory();
                }
            }
        }
...
//        }
    	...
    	*/
        public void trace(EventType type, String message) {
            log(Logger.TRACE, type, message, null);
        }
    	...
    	*/
        public void debug(EventType type, String message) {
            log(Logger.DEBUG, type, message, null);
        }
    	...
    	*/
        public void info(EventType type, String message) {
            log(Logger.INFO, type, message, null);
        }
    	...
    	*/
        public void warning(EventType type, String message) {
            log(Logger.WARNING, type, message, null);
        }
    	...
    	*/
        public void error(EventType type, String message) {
            log(Logger.ERROR, type, message, null);
        }
    	...
    	*/
         ...
         */
        private void log(int level, EventType type, String message, Throwable throwable) {
        	
        	
        	// Check to see if we need to log
//        	if (!jlogger.isLoggable( level )) return;
        	if (level < Logger.DEBUG && !isTraceEnabled()) {
        		return;
        	}
        	if (level < Logger.INFO && !isDebugEnabled()) {
        		return;
        	}
        	if (level < Logger.WARNING && !isInfoEnabled()) {
        		return;
        	}
        	if (level < Logger.ERROR && !isWarningEnabled()) {
        		return;
        	}
        	if (level >= Logger.ERROR && !isErrorEnabled()) {
        		return;
        	}
...
//			jlogger.log(level, "[" + typeInfo + getUserInfo() + " -> " + appInfo + "] " + clean, throwable);
        	...
        	if (level < Logger.DEBUG) {
        		jlogger.trace("[" + typeInfo + " -> " + appInfo + "] " + clean, throwable);
        	} else if (level < Logger.INFO) {
        		jlogger.debug("[" + typeInfo + " -> " + appInfo + "] " + clean, throwable);
        	} else if (level < Logger.WARNING) {
        		jlogger.info("[" + typeInfo + " -> " + appInfo + "] " + clean, throwable);
        	} else if (level < Logger.ERROR) {
        		jlogger.warn("[" + typeInfo + " -> " + appInfo + "] " + clean, throwable);
        	} else {
        		jlogger.error("[" + typeInfo + " -> " + appInfo + "] " + clean, throwable);
        	}
        }
...
//        }
		...
		public void always(EventType type, String message, Throwable throwable) {
            log(Logger.OFF, type, message, throwable);  // Seems backward, but this is what works, not Level.ALL	
		}
    }
}
