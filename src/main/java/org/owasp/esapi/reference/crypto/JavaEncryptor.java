...
/**
 * Reference implementation of the {@code Encryptor} interface. This
 * implementation layers on the JCE provided cryptographic package. Algorithms
 * used are configurable in the {@code ESAPI.properties} file. The main property
...
public final class JavaEncryptor implements Encryptor {
    ...
    /**
     * Generates a new strongly random secret key and salt that can be copy and
     * pasted in the <b>ESAPI.properties</b> file.
     *
     * @param args Set first argument to "-print" to display available
     * algorithms on standard output.
     * @throws java.lang.Exception	To cover a multitude of sins, mostly in
     * configuring ESAPI.properties.
     */
     ...
     *
     * @throws EncryptionException if can't construct this object for some
     * reason. Original exception will be attached as the 'cause'.
     */
     ...
     * {@inheritDoc}
     *
     * Hashes the data with the supplied salt and the number of iterations
     * specified in the ESAPI SecurityConfiguration.
     */
     ...
     * {@inheritDoc}
     *
     * Hashes the data using the specified algorithm and the Java MessageDigest
     * class. This method first adds the salt, a separator (":"), and the data,
     * and then rehashes the specified number of iterations in order to help
     ...
     * encryption key specified by the {@code Encryptor.MasterKey} property in
     * {@code ESAPI.properties}.
     *
     ...
     * {@code ESAPI.properties}.
     *
     * @param b64IVCiphertext	A base64-encoded representation of the IV + raw
     * ciphertext string to be decrypted with the default master key.
     ...
     * try { Mac hmac = Mac.getInstance("HMacSHA1"); // DISCUSS: Changed to
     * HMacSHA1. MD5 *badly* broken // SHA1 should really be avoided, but using
     * //	for HMAC-SHA1 is acceptable for now. Plan //	to migrate to SHA-256 or
     * NIST replacement for //	SHA1 in not too distant future. // DISCUSS: Also
     * not recommended that the HMac key is the same as the one //	used for
     * encryption (namely, Encryptor.MasterKey). If anything it //	would be
     * better to use Encryptor.MasterSalt for the HMac key, or //	perhaps a
     * derived key based on the master salt. (One could use //
     * KeyDerivationFunction.computeDerivedKey().) // byte[] salt =
     * ESAPI.securityConfiguration().getMasterSalt(); hmac.init( new
     * SecretKeySpec(salt, "HMacSHA1") );	// Was:	hmac.init(secretKeySpec)
     * byte[] inBytes; try { inBytes = input.getBytes("UTF-8"); } catch
     * (UnsupportedEncodingException e) {
     * logger.warning(Logger.SECURITY_FAILURE, "computeHMAC(): Can't find UTF-8
     * encoding; using default encoding", e); inBytes = input.getBytes(); }
     * byte[] bytes = hmac.doFinal( inBytes ); return
     * ESAPI.encoder().encodeForBase64(bytes, false); } catch
     * (InvalidKeyException ike) { throw new EncryptionException("Encryption
     * failure", "Must install unlimited strength crypto extension from Sun",
     * ike); } catch (NoSuchAlgorithmException e) { throw new
     * EncryptionException("Could not compute HMAC", "Can't find HMacSHA1
     * algorithm. " + "Problem computing HMAC for " + input, e ); } }
     * ******************
     */
    /**
     ...
     * {@code ESAPI.Encryptor.warnEveryNthUse}.) In other words, we nag them
     * until the give in and change it. ;-)
     *
